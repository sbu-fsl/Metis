...
for i in $(seq 0 $(($n_fs-1))); do
    ...
    then
        SDB_ID=$((S$WARM_ID * $ALL_SDBS + $SDB_CNT))
        SDB_CNT=$(($SDB_CNT+1))
        DEVLIST[$i]="/dev/sdb$SDB_ID"
    elif [ "$dev_type" = "" ]
    then
        DEVLIST[$i]=""
    else
...
setup_f2fs() {
    DEVFILE=$1;
    ramdisk_sz=$(runcmd blockdev --getsize64 $DEVFILE);
    echo "Your ramdisk device ($DEVFILE)'s size is $ramdisk_sz bytes."
    devsize=$(runcmd verify_device $DEVFILE f2fs $(expr 70 \* 1024 \* 1024))
...
setup_btrfs() {
    ...
    devsize=$(runcmd verify_device $DEVFILE btrfs 30777216);
    runcmd dd if=/dev/zero of=$DEVFILE bs=1k count=$(expr $devsize / 1024)
