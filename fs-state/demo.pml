...
proctype worker()
{
    ...
    :: pick_write_offset(offset);
       ...
       atomic {
        ...
        c_code {
            ...
            for (i = 0; i < N_FS; ++i) {
                makecall(rets[i], errs[i], "%d, %s, %p, %ld, %zu", write_file_in_kvm, i, testfiles[i], data,
    ...
    :: pick_truncate_len(filelen);
       atomic {
        ...
        c_code {
            ...
            for (i = 0; i < N_FS; ++i) {
                makecall(rets[i], errs[i], "%d, %s, %ld", truncate_file_in_kvm, i, testfiles[i], (off_t)Pworker->filelen);
                // fsfreeze(fslist[i], devlist[i], basepaths[i]);
                compute_abstract_state_in_kvm(i, basepaths[i], absfs[i]);
            }
            expect(compare_equality_fexists(fslist, N_FS, testfiles));
    ...
    :: atomic {
        ...
        c_code {
            ...
            //mountall();
            for (i = 0; i < N_FS; ++i) {
                makecall(rets[i], errs[i], "%d, %s", unlink_file_in_kvm, i, testfiles[i]);
                // fsfreeze(fslist[i], devlist[i], basepaths[i]);
                compute_abstract_state_in_kvm(i, basepaths[i], absfs[i]);
            }
            expect(compare_equality_fexists(fslist, N_FS, testfiles));
    ...
    :: atomic {
        ...
        c_code {
            ...
            //mountall();
            for (i = 0; i < N_FS; ++i) {
                makecall(rets[i], errs[i], "%d, %s, 0%o", create_dir_in_kvm, i, testdirs[i], 0755);
                // fsfreeze(fslist[i], devlist[i], basepaths[i]);
                compute_abstract_state_in_kvm(i, basepaths[i], absfs[i]);
            }
    ...
    :: atomic {
        ...
        c_code {
            ...
            //mountall();
            for (i = 0; i < N_FS; ++i) {
                makecall(rets[i], errs[i], "%d, %s", remove_dir_in_kvm, i, testdirs[i]);
                // fsfreeze(fslist[i], devlist[i], basepaths[i]);
                compute_abstract_state_in_kvm(i, basepaths[i], absfs[i]);
            }
            expect(compare_equality_fexists(fslist, N_FS, testdirs));
