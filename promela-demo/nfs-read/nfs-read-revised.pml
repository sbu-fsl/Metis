...
proctype NFS_SERVER(chan qrequest; chan qreply) {
    ...
    if
    /* AMBIGUITY 1: eof unclear in RFC; assumed "false" based on expertise */
    ::(count == 0) -> {
        printf("Case 1: read 0 byte\n");
        qreply ! NFS4_OK, false, 0, nondata;
        goto read_success
    }
    ::(count != 0) ->
        if ::(offset >= file_len) -> {
            ...
            ::(offset < file_len && offset + count >= file_len) -> {
